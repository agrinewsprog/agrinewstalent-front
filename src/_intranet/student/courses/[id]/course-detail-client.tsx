      ...
      case 'in-progress':
        return (
          <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
            {t('student.courses.statusInProgress', { progress })}
          </span>
        );
      case 'not-started':
        return (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="flex items-center gap-6 text-sm text-gray-600">
          <span className="flex items-center gap-2">
            ...
            {t('student.courses.duration', { duration: course.duration })}
          </span>
        </div>
      </div>
      ...
      {status !== 'not-started' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-medium text-gray-900">{t('student.courses.courseProgress')}</h3>
      ...
      {course.videoUrl && status !== 'not-started' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h3 className="font-medium text-gray-900 mb-4">{t('student.courses.courseContent')}</h3>
          <div className="aspect-video bg-gray-900 rounded-lg overflow-hidden">
            ...
            >
              {t('student.courses.videoNotSupported')}
            </video>
          </div>
        </div>
      )}
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="prose max-w-none">
            <p className="text-gray-600">
              {t('student.courses.courseContentPlaceholder')}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3">
        {status === 'not-started' && (
          ...
          >
            {isLoading ? t('student.courses.starting') : t('student.courses.startCourse')}
          </button>
        )}
        ...
        {status === 'in-progress' && progress < 100 && (
          ...
          >
            {isLoading ? t('student.courses.completing') : t('student.courses.markCompleted')}
          </button>
        )}
        ...
        {status === 'completed' && (
          <div className="flex items-center gap-2 text-green-600">
            ...
            <span className="font-medium">{t('student.courses.courseCompleted')}</span>
          </div>
        )}
      </div>
    </div>
