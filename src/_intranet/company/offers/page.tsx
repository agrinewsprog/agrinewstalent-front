...
export default function CompanyOffersPage() {
  ...
  const fetchOffers = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/api/offers/companies/me`, { credentials: 'include' });
      if (!response.ok) throw new Error();
      const data = await response.json();
      const raw: ApiOffer[] = data.offers ?? [];
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-green-600 text-white text-sm font-medium hover:bg-green-700 transition-colors shadow-sm"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="w-80 flex-shrink-0 flex flex-col">
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            {pagedOffers.length === 0 ? (
              <div className="text-center py-12 text-gray-500 text-sm">{t('company.offers.noOffers')}</div>
          ...
          {!selectedOffer ? (
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="pt-2 border-t border-gray-100 flex items-center gap-2">
                <span className="text-xs text-gray-500 font-medium">{t('common.status')}</span>
