...
export default function UniversityNotificationsPage() {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      setNotifications(unwrapNotifications(data, locale, 'university'));
    } catch (err) {
      if (IS_DEV) console.error('[UniversityNotifications] fetch error:', err);
    } finally {
      setLoading(false);
    }
  }, [locale]);
  ...
  const handleMarkAsRead = async (id: string) => {
    try {
      ...
      if (IS_DEV) console.error('[UniversityNotifications] markAsRead error:', err);
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">
          {t('university.notifications.title')}
        </h1>
        <p className="text-gray-600 mt-1">
          {t('university.notifications.subtitle')}
        </p>
      </div>
