...
export default async function CandidateDetailPage({ params }: { params: Promise<{ id: string; locale?: string }> }) {
  ...
  const app =
    applications.find((a) => resolveStudentId(a, a.student) === id) ??
    applications.find((a) => resolveApplicationId(a) === id);
  const resolvedStudentId = app ? resolveStudentId(app, app.student) : null;
  const allAppsFromStudent = resolvedStudentId
    ? applications.filter((a) => resolveStudentId(a, a.student) === resolvedStudentId)
    : app
      ? [app]
      : [];
  ...
  if (!app) {
    return (
      <div className="max-w-3xl mx-auto py-16 text-center">
        ...
        <Link href={buildLocaleHref(locale, '/intranet/company/candidates')} className="mt-4 inline-flex items-center gap-1.5 text-sm text-green-700 hover:text-green-900 transition-colors">
  ...
  const avatar = toAbsoluteAssetUrl(resolveAvatarUrl(student, app), API);
  const cvUrl = toAbsoluteAssetUrl(resolveResumeUrl(student, app), API);
  const name = resolveStudentDisplayName(student, app) || 'Candidato';
  ...
  const applicationSource = resolveApplicationSource(app, app.offer);
  const statusLabelRaw = t(`company.applicationStatus.${normalizedStatus}` as any);
  const statusLabel = (typeof statusLabelRaw === 'string' && statusLabelRaw.includes('applicationStatus.')) ? normalizedStatus : (statusLabelRaw || normalizedStatus);
  const statusColor = STATUS_COLOR[normalizedStatus] ?? 'bg-gray-100 text-gray-600 border-gray-200';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-10">
      <Link href={buildLocaleHref(locale, '/intranet/company/candidates')} className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900 transition-colors">
