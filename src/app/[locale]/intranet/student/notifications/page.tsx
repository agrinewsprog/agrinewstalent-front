...
export default function StudentNotificationsPage() {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      setNotifications(unwrapNotifications(data, locale, 'student'));
    } catch (err) {
      if (IS_DEV) console.error('[StudentNotifications] fetch error:', err);
    } finally {
      setLoading(false);
    }
  }, [locale]);
  ...
  const handleMarkAsRead = async (id: string) => {
    try {
      ...
      if (IS_DEV) console.error('[StudentNotifications] markAsRead error:', err);
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">
          {t('student.notifications.subtitle')}
        </p>
      </div>
