...
import { toAbsoluteAssetUrl } from '@/lib/frontend/contracts';
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden sticky top-4">
      ...
      <div className="px-6 py-5 space-y-4">
        ...
        {/* Acciones */}
        <Link
          href={buildStudentOfferHref(locale, item.offerId)}
...
export function SavedOffersClient({ initialItems, initialAppliedIds }: Props) {
  ...
  const handleApply = async (offerId: number) => {
    ...
    try {
      ...
      showError(err instanceof Error ? err.message : t('student.offers.notifications.applyError'));
    } finally {
      setApplyingId(null);
    }
  };
